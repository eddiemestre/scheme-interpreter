...
# primitive scheme prodecure
# self.fn - a function that implements the primitive Scheme procedure
# self.use_env - the environment it should use. Defaults to False to use
# global environment
...
# all of the @primitive funcions below are part of the globablly
# defined _PRIMITIVES list
_PRIMITIVES = []
...
# every element of a scheme list is ordered such that the first val
# is the value and the second value is either nil or a nested Pair
# the algorithm goes from the end of the list backwards to get the full scheme
# list -> the inside most Pair will be the last val in the list of vals
# paired with nil Pair(val, nil)
# then the second most inward layer will use that inside pair as its second
# element with the last-1 val -> Pair(val2, Pair(val, nil))
# this will continue until we've computed through the 0th element
...
def scheme_append(*vals):
    # if there are no vals to append, return nil
    if len(vals) == 0:
        return nil
    # grab the last value in the scheme list of values
    # you want to append to the existing list
    result = vals[-1]
    # while there are values remaining...
    for i in range(len(vals)-2, -1, -1):
        # get the value, starting with the second to last
        # value
        v = vals[i]
        # if that value is not nil
        if v is not nil:
            # check the type
            # v -> value we are checking
            # scheme_pairp -> predicate we are checking that applies to val
            # is v a scheme pair?
            # i -> specific arg used for error handling
            # "append" -> name of this function for error handling
...
# a string is any string that starts with quotes
@primitive("string?")
def scheme_stringp(x):
    return isinstance(x, str) and x.startswith('"')

# a symbol is anything that is a string and is not part
# of the scheme_stringp class (doesn't start with quotes)
@primitive("symbol?")
def scheme_symbolp(x):
    return isinstance(x, str) and not scheme_stringp(x)

# a number is any value x that is either an int or float value
@primitive("number?")
def scheme_numberp(x):
    return isinstance(x, int) or isinstance(x, float)

# an integer is a number that is either an int or a number where
# rounding x is the same value as x
@primitive("integer?")
def scheme_integerp(x):
    return isinstance(x, int) or (scheme_numberp(x) and round(x) == x)

# check_nums takes in a list of vals and checks if each one is a number
# enumerate to get the location and value of the vals
# raises a SchemeError if not all numbers are vals
...
# checks if value is a boloolean, number, or symbol
@primitive("atom?")
def scheme_atomp(x):
