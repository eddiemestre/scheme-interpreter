...
class Buffer:
    ...
    """
    # source is the entire program
    def __init__(self, source):
        self.index = 0
        self.lines = []
        self.source = source #map object tokenize_lines(input_lines) where input_lines is a LineReader instance
        self.current_line = ()
        self.current()
    ...
    def more_on_line(self):
        # ensures there is more on line or returns None
        return self.index < len(self.current_line)

    def current(self):
        """Return the current element, or None if none exists."""
        # try to get the current element by returning its place in the current_line
        while not self.more_on_line:
            # if there is nothing else on the current line, set the self.index to 0 (so next
            # time we grab a line, we read from the beginning of that line) and see if there 
            # is a next line to grab from the source
            # if there is, add it to the self.lines list of lines and then return the 
            # self.index (which will be 0) of the newly pulled line (aka the first element of 
            # the newly grabbed line)
            self.index = 0
            try:
    ...
    def __iter__(self):
        while self.lines:
            ...
            if (self.prompt is not None and line != "" and
                ...
                self.prompt = ' ' * len(self.prompt)
            # yield allows us to iterate over the next line immediately
            # instead of working through the whole function again
            # # yield sends a value back to the caller but retains enough
            # state to enable a function  to resume where it left off
            # generator
            yield line
        raise EOFError
