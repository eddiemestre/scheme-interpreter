...
# expr - scheme well-formed list with a .first and .second (rest) structure
...
def scheme_eval(expr, env):
    ...
    # Evaluate Atoms which includes numbers, booleans, nil and symbols
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        if len(procedure.formals) != len(args):
            raise SchemeError("Lambda formals of different length than corresponding arguments")  
        new_frame = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, new_frame)
    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        if len(procedure.formals) != len(args):
            raise SchemeError("Mu formals of different length than corresponding arguments")
...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Type Error detected in apply_primitive")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Formal list is not the same length as vals list")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            ...
            raise SchemeError("non-symbol {target.first}")
    else:
...
def do_quote_form(vals):
    ...
    check_form(vals, 1, 1)
    "*** YOUR CODE HERE ***"
    # when print is called in read_eval_print_loop, the str method will be called
    # which will print the nested Pairs properly
    return vals[0]
...
def do_let_form(vals, env):
    ...
    for i in bindings:
        if len(i) > 2:
            new_env.define(i.first, scheme_eval(Pair("begin", i.second), env))
        else:
            new_env.define(i.first, scheme_eval(i.second.first, env))
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    # vals = rest - the first pair needs to be evaluated
    # if true, return the eval of 2nd pair
    # if false, return the eval of the 3rd pair if there is one, otherwise
    # Return okay
...
def do_and_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    # range based for loop from 0 - end 
    # if scheme_true of evaluated expression, keep going
    # otherwise return false
    # return eval of the last element in the range
    last = len(vals) - 1

    if last < 0:    # empty and returns True
        return True
    ...
    return scheme_eval(vals[last], env)
...
def do_or_form(vals, env):
    ...
    if last < 0:    # empty or returns False
        return False
    
    for i in range(0, last):
        temp = scheme_eval(vals[i], env)
        if scheme_true(temp):
            # quote temp and return it
            return quote(temp)
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError("formals not a well-formed list.")
    
    formal_list = []
    for i in formals:
        if i in formal_list:
            raise SchemeError("{i} is duplicate symbol")
        if not scheme_symbolp(i):
            raise SchemeError("{i} in not a valid symbol")
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                raise SchemeError("Cannot call {str(procedure)}")
...
@main
def run(*argv):
    # grab the next line from the user
